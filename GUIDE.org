...
* Quick Reference — Keybinding Table

Custom bindings from init.el and critical package defaults.
...
| =C-c k=   | consult-ripgrep        | Search files  |
| =C-c g=   | consult-git-grep       | Search git    |
| =C-c j=   | consult-git-grep       | Search git    |
...
| =C-c p=   | project prefix map     | Project       |
| =C-c c=   | avy-goto-char-timer    | Jump to char  |
| =C-c s=   | avy-goto-line          | Jump to line  |
| =C-c f=   | apheleia-format-buffer | Format code   |
| =C-x g=   | magit-status           | Git status    |
| =C-c e=   | elfeed                 | RSS reader    |
| =C-c t=   | modus-themes-toggle    | Toggle theme  |
| =C-c I=   | find-user-init-file    | Open init.el  |
| =S-arrows= | windmove              | Switch window |

** Eglot Bindings (active in LSP buffers)
...
** Flymake Bindings (active when flymake is on)
